...
object ExampleHelpCommand {
    ...
    private val HELP_ID = ScathaUtils.MOD_ID.hashCode()
    ...
    private fun createCommandEntry(command: CommandBuilder): IChatComponent {
        ...
        val categoryDescription = category.description.replace("SkyHanni", ScathaUtils.MOD_NAME).splitLines(200).replace("§r", "§7")
    ...
    private fun showPage(page: Int, search: String, commands: List<CommandBuilder>) {
        ...
        val title = "${ScathaUtils.MOD_NAME} Commands" + if (search.isNotBlank()) "Matching: \"$search\"" else ""
